...
import { AppState, ContactForm, GrammarLevel } from '../types';
...
export const useAppController = () => {
  ...
  const setCurrentView = useCallback((view: 'home' | 'grammar-videos') => {
    setState(prev => ({ ...prev, currentView: view }));
  }, []);

  const getGrammarLevels = useCallback((): GrammarLevel[] => {
    return controller.getGrammarLevels(state.language);
  }, [controller, state.language]);
  return {
    state,
    actions: {
